...
# Builder stage
FROM eclipse-temurin:26.0.2_10-jre-noble AS builder
...
# Install PlantUML
RUN curl -sLOJ https://github.com/plantuml/plantuml/releases/download/v1.2026.8/plantuml.jar --output-dir /usr/local/bin/ && \
    echo '#!/bin/bash\njava -jar /usr/local/bin/plantuml.jar "$@"' > /usr/local/bin/plantuml && \
    chmod +x /usr/local/bin/plantuml

# Copy and setup Structurizr CLI
ADD structurizr.sh /structurizr-cli/structurizr.sh
RUN curl -sLJ https://download.structurizr.com/structurizr-2026.09.19.war --output-dir /structurizr-cli --output structurizr.war && \
